...
<script type="text/ecmascript-6">
    ...
    import {summaryTimePopUp, topicList, activeSummaryTopic, summaryRefreshTopic, summaryResetSearch } from '../../../../vuex/getters';
    import {setSummaryTimePopUp, setTopicList, setActiveSummaryTopic, setSummaryRefreshTopic, setSummaryResetSearch } from "../../../../vuex/actions";
    ...
    export default{
        ...
        components:{ Tips },
        vuex: {
            actions: {setSummaryTimePopUp, setTopicList, setActiveSummaryTopic, setSummaryRefreshTopic, setSummaryResetSearch },
            getters: {summaryTimePopUp, topicList, activeSummaryTopic, summaryRefreshTopic, summaryResetSearch }
        },
        methods: {
            toggle(group){
                //console.log(group);
            ...
            selectTopic(topic){
                ...
                console.log('old',(this.activeSummaryTopic).topic_id);
                if((this.activeSummaryTopic).topic_id==topic.topic_id){
                    this.setSummaryRefreshTopic(this.summaryRefreshTopic+1)
                    console.log('相同',this.summaryRefreshTopic);
                }else{
                    //this.setSummaryTimePopUp(0.33);
                    if(this.summaryResetSearch == false);{
                        console.log('判断不为空');
                        console.log('修改时间',(new Date()).getTime())
                        this.setSummaryResetSearch(true);
                    }
                    setTimeout(function () {
                        console.log('延迟')
                        ...
                        this.setSummaryRefreshTopic(0);
                        console.log('不同',this.summaryRefreshTopic);
                    }.bind(this),50);
                }
            },
            addTopicAction(){
        ...
        filters: {
            isActive(topic){
                return _.isEqual(this.activeSummaryTopic, topic) ? 'active' : '';
            }
        }
    }
</script>
