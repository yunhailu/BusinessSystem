<template>
    <header-component active="instruction"></header-component>
    <div class="instruction-wrap">
        <!--<img class="image" v-bind:src="imgUrl">-->
        <img v-if="" class="image" src="../../../../images/userInstruction.png">
        <!--<img v-if="" class="image" src="../../../../images/ccsiInstruction.jpg">-->
        <!--<img v-if="" class="image" src="../../../../images/mediaqInstruction.jpg">-->
        <!--<img v-if="" class="image" src="../../../../images/userInstruction.png">-->
        <!--<div class="about-title">
            <h2>使用说明</h2>
            <p class="blue">
                <span class="indx-gb">INSTRUCTIONS</span>
            </p>
        </div>
        <dl class="accountID">
            <dt>
                <h3>
                    <span>01</span>账号中心
                </h3>
            </dt>
            <dd>
                <h4>
                    <span>a)</span>登录
                </h4>
                <div><img src="../../../../images/intro-1.png"></div>
                <p>用户根据申请的账号登录</p>
            </dd>
        </dl>
        <dl class="accountID">
            <dt>
                <h3>
                    <span>02</span>情报检测方案设置
                </h3>
            </dt>
            <dd>
                <h4>
                    <span>a)</span>选择方案分组<br/>
                    <span>b)</span>设置监测词<br/>
                    <span>c)</span>预警值设置<br/>
                    <span>d)</span>带年纪开始监测，即方案设置成功
                </h4>
                <div><img src="../../../../images/intro-2.png"></div>
            </dd>
        </dl>

        <dl class="accountID">
            <dt>
                <h3>
                    <span>03</span>查看情报信息
                </h3>
            </dt>
            <dd>
                <p>检测方案成功，请选择数据分析至品牌关注页面，在该页面，您可以点击品牌关注、品牌情绪、品牌评价、品牌印象和品牌影响力等板块，查看对应数据及分析</p>
                <div><img src="../../../../images/intro-3.png"></div>
                <p>点击单条信息链接可转至单片文章详情页</p>
                <div><img src="../../../../images/intro-4.png"></div>
            </dd>
        </dl>

        <dl class="accountID">
            <dt>
            <h3>
                <span>04</span>情报预警
            </h3>
            </dt>
            <dd>
                <p>监测设置里修改方案里的情绪设置</p>
                <div><img src="../../../../images/intro-5.png"></div>
                <p>情报预警查看及发送邮件</p>
                <div><img src="../../../../images/intro-6.png"></div>
            </dd>
        </dl>

        <dl class="accountID">
            <dt>
            <h3>
                <span>05</span>报告生成
            </h3>
            </dt>
            <dd>
                <p>数据分析页面自定义添加快捷报告</p>
                <div><img src="../../../../images/intro-7.png"></div>
                <p>报告分类查看</p>
                <div><img src="../../../../images/intro-8.png"></div>
                <p>报告打印以及保存各种格式</p>
                <div><img src="../../../../images/intro-9.png"></div>
            </dd>
        </dl>-->
    </div>
</template>

<style lang="less">
    @import "InstructionsForUse.less";
</style>

<script type="text/ecmascript-6">
    import HeaderComponent from '../../Header/Header.vue';
    import Local from '../../../local/local';
    function stop(){
        return false;
    }
    window.document.oncontextmenu=stop;
    export default{
        data(){
            const instruction=Local().instruction;
            return {
                imgUrl:instruction.forUse
            }
        },
        components:{HeaderComponent}

    }
</script>